...
const PLANS = [
  {
    ...
    bullets: [
      'Bilan corporel initial',
      'Plan nutrition personnalisé',
      'Plan d’entraînement personnalisé',
      '2 appels coaching / mois',
  ...
  },
  {
    ...
    bullets: [
      'Tout Fondation',
      'Coaching hebdomadaire (4 / mois)',
      'Scanner d’assiette IA débloqué',
      'Analyse corporelle bimensuelle',
      'Tarif Membre sur tous les produits',
    ],
  },
  {
    ...
    bullets: [
      'Tout Ascension',
      'Coach élite en 1:1 dédié',
      'Protocole 100% sur mesure',
      'Bilan physiologique annuel',
      'Invitation retraite Société',
    ],
  },
];

function Subscription({ onJoin }) {
  return (
          ...
          {PLANS.map((p) => (
            <article key={p.id} className={`dt-sub-card dt-sub-${p.accent} ${p.featured ? 'is-featured' : ''} dt-reveal`}>
              ...
              <ul className="dt-sub-list">
                {p.bullets.map((b) => (
                  <li key={b}><i data-lucide="check"></i><span>{b}</span></li>
                ))}
              </ul>
              <button
                ...
                onClick={onJoin}
              >
                Rejoindre · {p.label} <i data-lucide="arrow-up-right"></i>
              </button>
            </article>
          ))}
        </div>
        <div className="dt-sub-foot dt-reveal">
          <span><i data-lucide="shield-check"></i>Satisfaction garantie 14 jours</span>
          <span><i data-lucide="users"></i>Admissions limitées chaque mois</span>
          <span><i data-lucide="phone"></i>Conseiller dédié au +216 71 000 000</span>
        </div>
      </div>
    </section>
  );
